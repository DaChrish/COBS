...
class TestSimulateDraftMulti:
    ...
    async def test_default_seed_uses_tournament_seed(self, client: AsyncClient):
        # With no seed in the request, both sims must fall back to the tournament
        # seed (reproduces the real draft) and therefore agree on round 1.
        admin = await client.post("/auth/admin/setup", json={"username": "admin", "password": "pw"})
        ah = {"Authorization": f"Bearer {admin.json()['access_token']}"}
        resp = await client.post("/test/tournament", json={"num_players": 16, "num_cubes": 6, "seed": 5}, headers=ah)
        tid = resp.json()["tournament_id"]

        single = await client.post(f"/tournaments/{tid}/simulate-draft", json={}, headers=ah)
        multi = await client.post(f"/tournaments/{tid}/simulate-draft-multi", json={"num_rounds": 1}, headers=ah)
        assert single.status_code == 201 and multi.status_code == 200

        # The resolved seed is surfaced (single: stored config, multi: response).
        assert single.json()["config"]["seed"] == 5
        assert multi.json()["seed"] == 5
